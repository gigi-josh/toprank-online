...
New conversation patterns not in jai_responses.py.
Focuses on location, work, plans, and natural back-and-forth.
"""

import random

class JAINatural:
    """Natural conversation — new patterns only"""
    
    @staticmethod
    def get_natural_response(message):
        """Generate responses for patterns not in main file"""
        msg = message.lower()
        
        # ========== ASKING ABOUT WORK (follow-up) ==========
        ...
        if any(l in msg for l in ["where are you", "where you at", "where you based", "you in"]):
            responses = [
        ...
        if any(l in msg for l in ["i'm in", "i am in", "based in", "relocated", "moved to", "came to", "camped at"]):
            if "sokoto" in msg or "camp" in msg:
        ...
        if any(c in msg for c in ["camp", "nysc", "serving", "service", "corps", "camped"]):
            responses = [
